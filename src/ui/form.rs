...
{
    let form_block = panel(" new ", app.current_focus == Focus::Form);
    let inner = form_block.inner(area).inner(Margin::new(2, 1));
    
    frame.render_widget(form_block, area);
    draw_form(frame, inner, &app.form_state, app.current_focus == Focus::Form);
}   


fn draw_form(frame: &mut Frame, area: Rect, form_state: & FormState, is_focused: bool) 
{
    ...
    field(frame, name_area, "Name", Field::Name, form_state, is_focused);
    field(frame, start_area, "Start", Field::Start, form_state, is_focused);
    field(frame, end_area, "End", Field::End, form_state, is_focused);
    field(frame, interval_area, "Interval", Field::Interval, form_state, is_focused);
}

//——— Helpers —————————————————————————————————————/

fn field(frame: &mut Frame, area: Rect, label: &str, which: Field, form: &FormState, panel_focused: bool)
{
    ...
    frame.render_widget(Paragraph::new(value.as_str()).style(field_style(field_focused, form.errors.contains(&which), panel_focused)), value_area_sized);
...
fn field_style(field_focused: bool, has_error:bool, panel_focused: bool) -> Style {
    let base = Style::new().italic();

    if !panel_focused {
        base
    } else if has_error {
        base.bg(Color::Rgb(0xE8, 0x8A, 0x8A)).fg(Color::Black)
    } else if field_focused {
        base.bg(Color::LightYellow).fg(Color::Black)
    } else {
        base.underlined()
    }
}